...
data <- read.csv("https://raw.githubusercontent.com/Eliza1494/Data-Mining/master/FinalProjectData/DataFinalProject_Final.csv")
library(tidyverse)
library(mosaic)
library(MASS) 
library(tidyverse)
...
data$made_in <- as.factor(data$made_in)
data$Fuel_Engine <- as.factor(data$Fuel_Engine)
data$transmission.type <- as.factor(data$transmission.type)
data$transmission.speed <- as.factor(data$transmission.speed)
data$drivetrain_ad <- as.factor(data$drivetrain_ad)
data$drivetrain_ad2 <- as.factor(data$drivetrain_ad2)
```

\section{Abstract} 

Constructing accurate statistical models to predict used car prices has been of high interest in numerous fields. Determining the resale value of a vehicle has been of a paramount importance for financial services, lenders and vehicle leasing services. This report examines number of distinct car specifications, that are considered to affect vehicle prices the most. To construct an accurate statistical model aimed at predicting used vehicle prices, four statistical techniques are applied: Multiple Linear Regression Analysis, Lasso, Random Forest and Boosting. The predictions are then examined and compared to decide which model provides the best performance. The report concludes with analysis of particular vehicle specifications, specifically fuel type, to examine and approximate their effect on used vehicle prices.  
...
type_price<-ggplot(data, aes(x=reorder(car_type, -price), y=price)) +
  ...
  labs(x = "car type", 
       y = "price")+
  scale_x_discrete(labels = c("Pickup Truck","Coupe","Convertible", "Utility", "Sedan", "Van/SUV", "Hatchback", "Wagon"))
...
Table 1.1 in the appendix represents all the 47 car models in the dataset. One of the goals in the paper is to find how much the type of the engine, and the fuel it supports, affect the value of the car. This is the reason why Figure 1.2 illustrates the different types of engine fuels that the models have in our dataset. Hence, the graphs below visually illustrate the relationship between listed price, vehicle models and engine fuel types.

Results from the graphs also indicate the current situation in the car industry: even though electric and hybrid vehicles are becoming more popular, there is still a large prevalence of regular vehicles. The internal combustion engine vehicles that are powered mainly by gas prevail in the dataset used. 

The vehicle models that also have both regular engine type fuel and hybrid/electric type are Audi, BMW, Buick, Cadillac, Chevrolet, Ford, GMC, Honda, Hyundai, Infiniti, Kia, Lexus, Lincoln, Mercedes-Benz, Mercury, Nissan, Porsche, Saturn, Smart, Subaru, Toyota, Volkswagen. However, the graph illustrates only these models that have the highest number of hybrid or electric vehicles to visually represent and analyze the price given engine type. The five models represented are BMW, Honda, Lexus, Nissan and Toyota.

Figure 1.2 represents a grid of five graphs to show the relationship between listed prices, vehicle models and engine types. The engine types here have been further grouped into Hybrid, Electric and Traditional. All engine fuel types beside hybrid and electric have been combined into Traditional to represent the regular type of vehicles. Even though Ford and Chevrolet have high amount of hybrid and electric vehicles, the regular type of engines exceed the hybrid and electric by so much that the visual representation is ambiguous, and these two models are excluded. The rest of the five vehicles models also have the potential to well represent the relationship between listed price and engine type since all five have high amount of electric and hybrid vehicles.

&nbsp;


```{r setup 1.1.5 grid, warning=FALSE, echo=FALSE, fig.width=18, fig.height=11.5}
...
The dataset used for estimate is an assembled one, joined from CarsXE and Craiglist based on unique VIN number. The Craiglist dataset was downloaded from the Kaggle platform (https://www.kaggle.com/austinreese/craigslist-carstrucks-data). However, this dataset has some caveats: many missing values, too few specifications of cars, and some values are inaccurate. To overcome these limitations, only valid vin numbers are extracted, which serve as the unique identity of a car, from the original dataset. Afterwards, these vin numbers are used to extract full specifications of cars from CarsXE (https://api.carsxe.com/), a database of cars with API access. After removing duplications and extreme values, a subsample in a size of approximately 24,000 cars was drawn to do our analysis. 

The cars data set is specifically tailored towards the southwest region in US, as to avoid different economic, policy factors affect the listed prices. The listed prices are as of January 2020 and posting made no later than a month prior to January 2020 as to avoid economic, political events that may have occurred in time that would affect demand/supply and oil prices. 

After data cleaning and manipulation the following variables are used for the analysis that follows. The car size, car type, transmission speed and type, drivetrain and fuel engine have been manipulated in a way to reduce the levels of the categorical variables.

  * Price: listed price of a car as of January, 2020 
...
To perform Lasso regression, the data is scaled first. The model is ran based on the “poisson” distribution which not only resembled the non-parametric distribution of price but also gave us a slightly lower error rate compared to “guassian”. A cross-validation is performed and Residual Sum of Squares is computed for each lambda. 

Following the default rule, lambda.1se is chosen instead of the minimum lambda in prediction, since such lambda is likely to lead to a more parsimonious model. The lamda.1se prepresents lambda within 1 standard error from the optimal lambda. By comparing the results of lambda.1se and lambda.min, no significant difference in estimated coefficients or error rates is presented between them.
...
X1$make <- as.factor(X1$make)
X1$car_type <- as.factor(X1$car_type)
X1$car_size <- as.factor(X1$car_size)
X1$made_in <- as.factor(X1$made_in)
X1$Fuel_Engine <- as.factor(X1$Fuel_Engine)
X1$transmission.type <- as.factor(X1$transmission.type)
X1$transmission.speed <- as.factor(X1$transmission.speed)
X1$drivetrain_ad <- as.factor(X1$drivetrain_ad)
X1$drivetrain_ad2 <- as.factor(X1$drivetrain_ad2)
...
title("Figure 1.3: Random Forest Predicted Values of Price and\n Actual Price Values")
```

&nbsp;


Applying the Random Forest model, it can be examined how on average price will change if cars fall in different categories such as models, engine, and fuel type. Given the difficulty of interpreting results from Random Forest, the method applied here and on the rest of prediction analysis is to predict prices on all cars that fall under a specific category or multiple categories and compute the average of these predicted prices. The limitation of this approach is that not all the variables are accounted for, hence, conclusions of the exact effect of a specific category cannot be determined. A solution to this caveat can be to specify as many as possible variables such as model, year, fuel and engine type and range of odometer.
...
The two standard lambda values used most commonly for boosting are 0.1 and 0.2. Here the shrinkage parameter is set at 0.2 since it gives better MSE performance than 0.1
...
The error rates (averaged RMSE) of the four statistical learning methods in 10 - fold cross validations are presented in the Table 4.1 below.

According to the error rates, tree-based models: Random Forest and Boosting perform better on average than both Lasso and the Linear Model. Also, Random Forest performs on average slightly better than Boosting. So, the estimates from both random forest and boosting are used to summarize the effects of features on prices. However, consideration is made to Lasso as well because Lasso not only showed us an improvement over the linear model, but also has the advantage that it incurs less computation burden than two tree-based models.
...
Mean_RMSE=c("Linear"=mean(Loocv$rmse_linear),"Lasso"=mean(Loocv$rmse_lasso),
             "Boosting"=mean(Loocv$rmse_boost), "Random_Forest"=mean(Loocv$rmse_RandomForest))
Mean_RMSE <- as.data.frame(`Mean_RMSE`)
kable(Mean_RMSE)
```
...
```{r setup partial effects graph, echo = FALSE, warning=FALSE, fig.width=14, fig.height=8}
...
grid.arrange(p1, p2, p3, p4, p5,layout_matrix = rbind(c(1,2),c(3,4), c(5,5)), 
             top = textGrob("Figure 1.4: Partial Effects of Top 5 Important Variables", 
              hjust = 0.5, gp=gpar(fontsize=15), vjust = 0.1))
```
...
yhat_forest14=predict(CarsRandomForest ,newdata = car_train[car_train$make == "BMW" & car_train$Fuel_Engine == "Electric" & car_train$odometer %between% c(10000, 40000) & car_train$year_factor == "2017",])
M14 = mean(yhat_forest14)

yhat_forest15=predict(CarsRandomForest ,newdata = car_train[car_train$make == "BMW" & car_train$Fuel_Engine == "Premium Unleaded Gas" & car_train$odometer %between% c(10000, 40000) & car_train$year_factor == "2017",])
M15 = mean(yhat_forest15)

yhat_forest16=predict(CarsRandomForest ,newdata = car_train[car_train$make == "BMW" & car_train$Fuel_Engine == "Electric" & 
                                                              car_train$odometer %between% c(30000, 60000) & car_train$year_factor == "2016",])
M16 = mean(yhat_forest16)

yhat_forest17=predict(CarsRandomForest ,newdata = car_train[car_train$make == "BMW" & car_train$Fuel_Engine == "Premium Unleaded Gas" & 
                                                              car_train$odometer %between% c(30000, 60000) & car_train$year_factor == "2016",])
M17 = mean(yhat_forest17)
...
`Average Predicted Prices` = c("2017 Electric BMW with odometer b/w 10000 and 40000" = mean(yhat_forest14),
                               "2017 Gas BMW with odometer b/w 10000 and 40000" = mean(yhat_forest15),
                               "2016 Electric BWM with odometer b/w 30000 and 60000" = mean(yhat_forest16),
                               "2016 Gas BWM with odometer b/w 30000 and 60000" = mean(yhat_forest17),
                                "2017 Electric models with odometer b/w 20000 and 40000" =
                                 mean(yhat_forest6), "2017 Gas models with odometer b/w 20000 and 40000" =
                                 mean(yhat_forest7),"2017 Electric models with odometer over 40000" =
                                 mean(yhat_forest8), "2017 Gas models with odometer over 40000" =
                                 mean(yhat_forest9),
                               "2017 Electric/Hybrid Chevrolet with odometer b/w 20000 and 40000" =
                                 mean(yhat_forest10), "2017 Gas Chevrolet with odometer b/w 20000 and 40000" =
                                 mean(yhat_forest11), "2017 Electric/Hybrid Chevrolet with odometer over 40000" = mean(yhat_forest12),
                               "2017 Regular Gas Chevrolet with odometer over 40000" =
                                 mean(yhat_forest13))
AvgPrice = as.data.frame(`Average Predicted Prices`)
AvgPrice$`Average Predicted Prices` <- round(AvgPrice$`Average Predicted Prices`)
kable(AvgPrice)
